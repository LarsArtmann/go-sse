...
- `scripts/smoke-examples.sh` + CI `Example smoke` job — boots each example server and requires real SSE bytes (root `/events` + `/broadcast`, datastar index + `/events`, htmx index + `/sse-container` + `/events`), catching dead handlers, wrong ports, and startup panics that the compile-only examples job cannot see. The three examples now honor `PORT` (defaults unchanged: 8080/8765/8766) so the script can use collision-proof ports.
- Weekly `go-datastar compat` workflow (Mondays 05:00 UTC + manual dispatch): clones go-datastar, bumps its go-sse/ssetest pins to the latest tags, and runs both its modules' tests — breaking changes surface red and early instead of during a hand-run upgrade weeks later. Dry-run-verified locally (clone → bump → green) before the workflow shipped.
- CI concurrency group (`ci-${{ github.ref }}`, cancel-in-progress) — a superseded push no longer burns runner minutes running its jobs to completion.
- `scripts/verify.sh` now cross-checks the golangci-lint version pin in `.github/workflows/ci.yml` against the local (nixpkgs) binary and fails with the exact fix on skew — the 2.12/2.13 class of red-master cannot recur silently.
- `SECURITY.md` — private vulnerability reporting via GitHub security advisories, scope, and disclosure expectations.
- `docs/guides/eventstore-patterns.md` — retention/GC patterns for `EventStore` implementations (ring buffer, TTL, persistent store, stale-ID semantics, reconnect-budget checklist).
- `docs/guides/filters-and-fanout.md` — `SubscribeFilter` recipes under the read-lock contract (per-connection topics, payload-prefix-only inspection, filter-reject vs. buffer-drop, `ReplayFiltered` pairing, anti-pattern checklist).
- CONTRIBUTING release checklist: pre-release fuzz-budget step (5m per target), tag-signing policy (`-s` when a signing key is configured, else `-a`, never "sign later"), scripted proxy probe, and post-release pin-refresh notes (govulncheck pin, go-datastar ssetest pin bump ownership).
