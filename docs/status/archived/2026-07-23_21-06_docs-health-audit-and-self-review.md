...
- Fully read 2026-08-29 (247/247 lines). The existing Resolution Appendix (added by the 21:23 follow-up) re-verified against current state: LICENSE is MIT, `GOWORK=off` documented in AGENTS.md, flake.nix + CI + examples + fuzz tests all exist, TODO_LIST/ROADMAP live and maintained. Its §f 1-31 all shipped across v0.1.0-v0.5.x; §f 32-50 feature considerations shipped (buffer, SendJSON, shutdown, Event.String, drop-policy docs, EventStore example, LastEventID validation) or live in ROADMAP §1-5/parked decisions. §E 7-9 code smells were fixed in the M7 cleanup (2026-07-23_22-33). Fully resolved -> archived.
