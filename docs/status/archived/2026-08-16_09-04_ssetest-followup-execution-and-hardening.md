...
- **§b.1** (CI validation) — first real CI runs happened with the v0.5.1/ssetest v0.1.0 push (`ec43574` lineage); CI has run test/vet/lint/coverage/vulncheck/fuzz over ssetest since.
- **§b.2** (corpus growth uncommitted) — still open → TODO_LIST.md (Parser parity & fuzz depth).
- **§b.3** — resolved: `ssetest/v0.1.0` + `ssetest/v0.2.0` tags exist; `go get github.com/larsartmann/go-sse/ssetest` works (verified on pkg.go.dev 2026-08-29).
- **§c verdicts:** 1 done (`ec43574` — release train answered: v0.5.1 + ssetest v0.1.0). 2 → deliberately kept duplicated; policy documented in AGENTS.md. 3 done at `d6bea20` (`roundtrip_test.go` + `FuzzWriteReadRoundTrip`). 4 → cqrs-htmx scope. 5 → open → TODO_LIST (testing/synctest). 6 → go-datastar TODO_LIST. 7 done (docs-health pass 2026-08-29 — this harvest).
- **§f verdicts:** 1 done (`ec43574`). 2 → open → TODO_LIST (Coverage). 3 → open → TODO_LIST (fuzz seeds). 4 → open → TODO_LIST (fuzz seeds). 5 done (CI green on pushes since v0.5.1). 6 → open → TODO_LIST (gopls hygiene). 7 done at `d6bea20`. 8 done (`ssetest/v0.1.0`, `ssetest/v0.2.0` + go-sse v0.5.1 subtree). 9 → open → TODO_LIST. 10 done (CI pins golangci-lint v2.12.2 to match flake). 11 done (docs-health pass — this harvest). 12 → go-datastar scope.
