...
Re-verified: 9.7MB binary removed + gitignored (`2c029b4`); HARVEST executed across subsequent sweeps and re-harvested in this pass; samber/do Option C shipped in v0.4.0 (`Shutdown`, `Health`, `WithBufferSize`); ROADMAP raw idea §5 present with the brainstorming link; c.3 gopls warning documented as false positive; c.6 anchor links cosmetic-only. The existing Resolution appendix remains accurate.
