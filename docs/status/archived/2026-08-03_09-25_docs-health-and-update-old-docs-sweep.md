...
Re-verified: §c items all shipped in v0.4.0 (`TestIntegration_ReplayFiltered`, `TestSubscribeFilter_ShutdownDrainsFilteredSubscribers`, race-test strengthening — later made deterministic in v0.5.1 at `1805b4e`, panic contract documented on `ReplayFiltered`), release cut (v0.4.0 and successors through v0.5.1), tree pushed. DiscordSync/cqrs-htmx items remain those repos' scope. The existing Resolution appendix above remains accurate.
