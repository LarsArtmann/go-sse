...
- Fully read 2026-08-29 (270/270). Resolution appendix (2026-07-26) verified: Q1 confirmed (flake check builds and runs the doCheck derivation - re-verified green this pass), Q2 stands (GOWORK=off permanent workaround), Q3 superseded (tags through v0.5.1 + ssetest v0.1.0/v0.2.0 on origin). Section F 1-4 fixed by later sweeps (file:line refs replaced with symbol/test-name evidence; counts measured not hardcoded; flake check runs 2 hermetic checks today; spec URL pinned by the WPT corpus work). Section F 5-25 shipped across v0.2.x-v0.5.x or superseded (FEATURES evidence format evolved past line refs entirely). Fully resolved -> archived.
