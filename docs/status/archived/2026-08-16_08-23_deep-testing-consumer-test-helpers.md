...
- **§2/§3 (docs wiring, NOT STARTED list)** — all done by the follow-up session (`25fd03f`, `1f65c5a`, `f64a8d1` lineage): ssetest README, CHANGELOG/FEATURES/AGENTS/README wiring, local lint run, flake check, erraudit pass, module_boundary_test.go, dataless-frame seeds, go.mod pin, TrimSuffix removal, actionlint.
- **§6 verdicts (50 items):** 1–21 done by `2026-08-16_09-04` session (see its §a; 20 → open → TODO_LIST coverage-gate ssetest threshold). 22 → Won't implement — no consumer ask; `WithPath` covers query strings. 23 done (`example_test.go` godoc examples). 24 → open → TODO_LIST (Docs). 25–29 → open, consolidated → TODO_LIST (ssetest e2e expansions folded into Parser parity & Coverage items where still relevant; OnDrop e2e blocked on safeDropCall decision). 30 → Won't implement — micro-optimization doc; benchmark exists in code. 31 done at `d6bea20`. 32–34 done (`ssetest/v0.1.0`, `ssetest/v0.2.0`, go-sse v0.5.1/v0.5.0 tags; pkg.go.dev verified 2026-08-29). 35 → Won't implement — matrix maintenance cost > value at current consumer count. 36–37 → cqrs-htmx scope. 38 → go-datastar release notes. 39 → Won't implement — Ginkgo example deferred until a Ginkgo consumer exists. 40 → open, minor → example VERIFY.md is example-scope. 41–43 → go-datastar scope (41), done (42 — `MustReadNEvents` covered), Won't-until-asked (43). 44 → Won't implement — doc table exists in ssetest README (`Event.Data` vs `DataLines`). 45 → Won't implement — `CollectWithTimeout` fail-on-empty is the pinned contract. 46 n/a. 47 → CI-enforced upstream. 48 done (CI vulncheck job covers ssetest). 49 → open → TODO_LIST (Docs: guides). 50 done — superseded by this pass.
- **§7 answers:** Q1 → releases cut 2026-08-22 (`ec43574`). Q2 → shipped as bug fix in datastartest v0.3.0 (go-datastar). Q3 → kept deliberately duplicated; policy in AGENTS.md.
