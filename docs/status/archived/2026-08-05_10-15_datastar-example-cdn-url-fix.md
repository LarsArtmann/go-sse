...
- **§c verdicts:** 11 (SRI) + 35 (CDN pinning) — **obsolete**: the DataStar JS bundle is now vendored (`example/datastar/static/datastar.js`, v1.0.2, go:embed) — no third-party origin remains to pin. 12 (nix overlay/launcher) → **Won't-until-asked** (example-scope polish). 13 (integration test for CDN URL) — **obsolete** with vendoring (no external URL in the served HTML). 14 (port flag) → **Won't implement** — rejected to keep the example minimal; never re-requested. 15 (browser E2E) → BLOCKED, TODO_LIST (scope decision).
- **§d.16** (auto-commit message) — left as-is per history-rewrite policy; this appendix is the correction of record.
- **§d.17 / §f.31** (stale running binary) — moot; the example was rebuilt/re-verified in the browser on 2026-08-05.
- **§f verdicts:** 32 (example HTML test) — **obsolete** with vendoring; example-scope tests landed later in main_test.go. 33 (amend message) → **Won't implement** (history policy; correction of record here). 34 → obsolete (SRI moot). 36 (flake app) → Won't-until-asked. 37 (link-check CI) → open, minor → folded into TODO_LIST CI items. 38 (chromedp smoke) → BLOCKED, TODO_LIST. 39 (json v2 in example) → **Won't implement** — kept v2 deliberately (documented false-positive investigation, 2026-08-03_00-51 §b.4; example builds under the repo's required GOEXPERIMENT). 40 done (`05-46`: eventPatchElements/eventPatchSignals constants). 41 (example build tag/module) → **Won't implement** — example stays in-root by design. 42–50 → Won't-until-asked / YAGNI / superseded (templ migration landed 2026-08-06; vendoring landed 2026-08-06). 51 (annotate old reports) — done across the 2026-08-03 sweeps and this pass. 52–55 → YAGNI.
