...
**Superseded by `2026-08-07_18-51_fix-plan-complete-and-final-verification.md`** — all Tier 3/4 items listed here as NOT STARTED were completed in that session (F5-3 `e2e_test.go`, F6-1 real vendorHash, F6-2 README JoinLines at `da9fc99`, F6-3/F6-4 verification, F7-1 ADR, F7-2 AGENTS, F7-3 CHANGELOG), and the two failing patch tests were corrected to `datastar-patch-elements` as this report's §b root-cause analysis prescribed. §d.2's `EventTypeExecuteScript` question was resolved: the constant is dead and was not shipped; ScriptPatch emits `datastar-patch-elements` by design (documented in go-datastar). Remaining §f items → go-datastar/cqrs-htmx repos.
