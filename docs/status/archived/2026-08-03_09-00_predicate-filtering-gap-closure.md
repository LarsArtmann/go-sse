...
Re-verified: §c items — ReplayFiltered HTTP integration test shipped (v0.4.0 `TestIntegration_ReplayFiltered`), Shutdown+filtered test shipped (v0.4.0), panic contract documented on `ReplayFiltered` (v0.4.0, corrected wording in v0.5.0), v0.4.0 cut, pushed. §f 1–12 shipped in v0.4.0; 13–17 done (doc.go sections, ROADMAP raw idea updated, example usage via the DataStar example); 18–22 release items done (v0.4.0, superseded by v0.5.x); 23–26 consumer items → cqrs-htmx/DiscordSync repos; 27–32 → Won't-until-asked API ideas (FilteredBroadcaster/WithFilter/meta-filtering never requested); 33–38 done or Won't; 39–50 → Won't-until-asked micro-coverage ideas (several landed incidentally: filtered-subscriber counts via Health/SubscriberCount tests). Q1 → DiscordSync left to its owner; Q2 → v0.4.0 was cut 2026-08-03; Q3 → policy REVERSED in v0.4.0 (`b666ed5`): panics are recovered and treated as non-matches (inline-corrected in the Q3 section by the 19-57 session).
