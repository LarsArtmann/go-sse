...
- **§b items closed:** concurrent fan-out test done (`05-46` session, deterministic multi-client test); graceful-shutdown path tested (`05-46` + main_test.go); DataStar patching verified in a real browser 2026-08-05 (`2026-08-05_10-15_datastar-example-cdn-url-fix.md`, recorded in TODO_LIST ✅ note).
- **§c verdicts:** example tests done (`05-46`: memStore/producer/filter/shutdown tests in main_test.go). nix flake check + nix fmt — done in same-day sessions and re-verified 2026-08-29. Browser E2E automation → BLOCKED, TODO_LIST (scope decision pending).
- **§f verdicts:** 1–7 done (`03-37`/`05-46` sessions: concurrent test, browser verify, shutdown test, flake check, fmt, memStore tests, structured-category filter). 8–14 done (`03-37`/`05-46`: file split, $replayed reset, banner fade, TOCTOU comment, O(n) comment, IdleTimeout, magic-number constants). 15–22 done (`03-37`: connection dot, empty state, scroll-to-top, pause, timestamp filter → removed as dead code in `05-46` (**Won't implement**), event count, favicon, OG tags). 23–26 done (`03-37` §a Tier 4). 27–50 future enhancements → **Won't-until-asked / YAGNI** (example-scope; ReplayFiltered demo, metrics endpoint, etc. remain ideas).
