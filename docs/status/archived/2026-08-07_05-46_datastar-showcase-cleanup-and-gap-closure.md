...
12. **The prior session's status report (`docs/status/archived/2026-08-07_03-37_*.md`) still describes dead code as "partially done".** It's a point-in-time snapshot and shouldn't be edited, but a reader following the trail might be confused.
...
- **§a** — verified intact (dead code removed, scroll-to-top shipped, CSP-safe external app.js, 3 new tests, lint clean).
- **§c verdicts:** 1 (Mermaid) → **Won't implement** — pkg.go.dev does not render Mermaid; ASCII stands. 2–7 (POST form, SQLite store, /health, ReplayFiltered demo, Prometheus, Dockerfile) → **Won't implement / YAGNI** — example-scope enhancements; revisit only on consumer ask. 8 (htmx `contextcheck`) — resolved: the nolint carries an explanatory reason (accepted pattern, same as the datastar handlers); lint is 0 issues repo-wide (verified 2026-08-29).
- **§f verdicts:** 1–3 (htmx lint/tests) — lint resolved as above; htmx example tests → Won't-until-asked (example-scope). 4–9 → open-ish example enhancements → **Won't-until-asked** (deterministic tests for filter/CORS/replay landed in main_test.go per §a). 10+ → YAGNI/example-scope.
