...
Re-verified: every §B/§C gap was closed by the same-day gap-closure session (`2026-08-03_09-00`) and v0.4.0 — FEATURES/DOMAIN_LANGUAGE/README/broadcaster-doc updates, predicate-overhead benchmark, HTTP integration test, BroadcastMany+filter test, panic-safety test, race-test strengthening (deterministic since v0.5.1). §F verdicts: 1 → **Won't implement** — DiscordSync is its own repo; the stray commits are its owner's call. 2–5 done (v0.4.0/09-00). 6–12 done (v0.4.0). 13–16 → cqrs-htmx/DiscordSync repos; TODO_LIST entry was added and later consumed. 17–23 → **Won't-until-asked** API ideas. 24–27 done (v0.4.0 tagged; ROADMAP updated; migration guidance covered by README + example). 28–33 done (AGENTS/doc.go/ExampleReplayFiltered). 34–37 done (WithBufferSize v0.4.0, Shutdown v0.4.0, Health v0.4.0, OnDrop v0.5.0). 38+ future items → shipped (buffer/shutdown/health/drop) or Won't-until-asked (backpressure, metrics helpers → ROADMAP §1).
