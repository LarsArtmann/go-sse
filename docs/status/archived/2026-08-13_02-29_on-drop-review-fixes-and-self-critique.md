...
- **GAP 1 (`safeDropCall`)** — still NOT implemented (verified: `fanout.go` has only `safePredCall`). → TODO_LIST.md (Correctness & safety).
- **GAP 2 (re-entrancy doc)** — still NOT documented. → TODO_LIST.md (Correctness & safety), together with the Broadcast/BroadcastMany cross-reference items from §b.
- **§c verdicts:** 1–2 (flake check, nix fmt) done (docs-health pass 2026-08-29 — full gate re-run green). 3 (govulncheck) done — CI vulncheck job runs it every push.
- **§f verdicts:** 1–4 → TODO_LIST (1–2) / done (3–4). 5–9 → TODO_LIST (with GAP 1/2). 10 → TODO_LIST (safeDropCall test). 11 → Won't implement — comment/example-level guidance only. 12–13 → TODO_LIST (clear-callback tests). 14 done at `46622c8` (`TestWithOnDrop_FiresPerSubscriber`, `TestWithOnDrop_BroadcastMany`). 15–16 → Won't-until-asked (race registration is doc'd as safe; drain-phase drop firing is unobserved in practice). 17–21 → design-decision items → ROADMAP §1 observability explorations (drop counters already listed). 22 → known gopls friction → TODO_LIST. 23 done (CI vulncheck). 24 → Won't implement — behavior verified intentional (same RLock). 25–27 → example-scope, Won't-until-asked. 28 done at `ccceeaa` (v0.5.0 CHANGELOG). 29 done (README Non-Blocking Drop Policy section references the drop policy; onDrop docs in v0.5.0). 30 → Won't implement — lint/docs review catches this adequately.
- **§g answers realized:** Q1 → decided as TODO_LIST item (recover, matching predicates — pending implementation). Q2 → not changed; `func(T)` stands. Q3 → shipped as separate commits (`46622c8`, `4673fed`).
