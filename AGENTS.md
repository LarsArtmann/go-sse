...
| Test helpers      | `ssetest/` (separate module)   | Consumer-side E2E testing: SSE wire parser (`ReadEvents`/`ReadNEvents`, WHATWG § 9.2.6-conformant, pinned by the transcribed WPT corpus in `wpt_format_corpus_test.go`), `StreamReader` for one-event-at-a-time reads on a live stream, `Collect*` HTTP helpers, request options, `Require*` assertions. go-datastar's `datastartest` is a thin wrapper that delegates its SSE parsing here. |
...
- **`datastartest` is a thin wrapper over `ssetest`, not a duplicate** (corrected 2026-09-03; the earlier "independent implementations, do not depend on each other" claim was stale — `datastartest/go.mod` requires `go-sse/ssetest` and its `reader.go` delegates via `ssetest.ReadEvents`/`ReadNEvents` plus a `[]ssetest.Event` conversion, as its doc.go states). Consequences: parser fixes ship ONCE, in `ssetest` — there is no apply-to-both mandate; the real coupling is release order (cut ssetest, then bump go-datastar's pin and tag datastartest — the pairing rule v0.6.0/v0.3.0 followed); and datastartest is not a single-dependency module. The 2026-08-16 WHATWG § 9.2.6 conformance fixes (CR/LF/CRLF split, BOM strip-once, sticky id/retry, NUL-id ignore, EOF discard) live in `ssetest/reader.go`; datastartest inherits them through its pin.
...
- **gopls noise in this repo (documented, do not chase):** (1) the `stdversion` warnings on `encoding/json/v2` DISAPPEARED with the Go 1.27 / `go 1.27.1` directive bump (2026-09-19); if they return, the toolchain regressed below the directive. (2) residual "unnecessary type argument" hints — all inferable sites were cleaned 2026-08-29; the rest are required (`NewBroadcaster[int]()` has no args; `WithBufferSize[T]`'s T lives only in the return type). `gopls check` CLI reports 0. Note: a gopls/golangci-lint LSP running on the AMBIENT 1.26.7 toolchain reports a spurious `go.mod requires go >= 1.27.1` failure — run tools inside `nix develop`, whose `go_1_27` matches the directive; trust the CLI run over the stale editor diagnostic.
...
- **CI now covers what it used to miss (2026-08-29):** examples build + `templ generate -check` (via `go run github.com/a-h/templ/cmd/templ@v0.3.1020` — `@version` form, because module-mode `go run` fails on templ's cmd deps missing from go.sum), `nix flake check` job (install-nix-action pinned by SHA), 6 fuzz targets, `govulncheck@v1.7.0` pinned (was `@latest`). The `.github/workflows/ci.yml` matrix runs 8 jobs — Test, Lint, Vet, Coverage, Examples, Nix flake check, Vulncheck, Fuzz — for both modules (plus `actionlint` and the scheduled `Flake update` in their own workflows).
...
- **`nix run .#coverage-gate` survives a broken caller `GOCACHE` since 2026-09-19:** the app validates `GOCACHE` (mkdir -p probe), falls back to a temp dir with a loud message, and no longer swallows `go test` stderr — a failing test run prints its log instead of dying as a silent exit 1 (the old behavior when a shell leaked `GOCACHE=/mnt/...`).
