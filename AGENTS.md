...
- **`nix run .#coverage-gate` exits 1 SILENTLY when the caller's `GOCACHE` points at a nonexistent path** (e.g. a shell leaking `GOCACHE=/mnt/...`). The app exports its own `GOWORK`/`GOEXPERIMENT` but not `GOCACHE`. Run it as `GOCACHE=/tmp/go-build-cache GOMODCACHE=… nix run .#coverage-gate`.
