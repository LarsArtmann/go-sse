...
| Test helpers      | `ssetest/` (separate module)   | Consumer-side E2E testing: SSE wire parser (`ReadEvents`/`ReadNEvents`, WHATWG § 9.2.6-conformant, pinned by the transcribed WPT corpus in `wpt_format_corpus_test.go`), `Collect*` HTTP helpers, request options, `Require*` assertions. Mirrors go-datastar's `datastartest`. |
...
**No retry/backoff loop** (evaluated 2026-08-07, declined). go-sse has no retryable operation: a failed `Stream.Send` means a dead or partially-written connection, so retrying re-emits bytes already on the wire and corrupts SSE frames — and it would sleep while holding `Stream.mu`, stalling `Heartbeat`. `EventStore` retry policy belongs to the consumer's store; the `Shutdown` drain is a condition-wait where backoff only raises shutdown latency; `Broadcast`'s drop-on-full is invariant #2. **Trap to remember:** go-sse marks connection-death errors (`sse.send_failed`, `sse.write_failed`, `sse.replay_failed`) as `Transient`, and `errorfamily.IsRetryable` returns true for exactly `Transient` — so any default-configured retry wrapper would retry a broken pipe. Here `Transient` means "drop the connection and let the browser reconnect", not "try the same socket again". Note also that `Event.Retry`/`WriteRetry` already own the word "retry" in this package with an unrelated meaning (a browser reconnect hint in ms). Full analysis and re-open triggers: [docs/brainstorming/2026-08-07_go-retry-adoption-evaluation.md](docs/brainstorming/2026-08-07_go-retry-adoption-evaluation.md).
