...
- **Auto-git commit messages can be plausible-but-false — review them after the fact.** The background commit daemon writes messages from diffs it does not understand; commit `38e79aa` says "expand test coverage" for a deletion that *reduced* coverage. After a session that deleted code or tests, run `git log --stat -5` and sanity-check the daemon's messages; published history stays unrewritten, but contradictions get recorded in the next status report so `git log` readers are not misled forever.
- **Status reports follow the conventions in [`docs/status/AGENTS.md`](docs/status/AGENTS.md)** — including the mandatory coverage-delta line (`- cover: library X% (+/-Δ), ssetest Y% (+/-Δ)`, measured this session, never quoted from memory). Read that file before writing any `docs/status/<date>_*.md` report.
